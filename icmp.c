...
icmp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    case ICMP_TYPE_ECHO:
        ...
        break;
    default:
        errorf("unsupported ICMP type=%u", hdr->type);
        break;
    }
}

int
icmp_output(uint8_t type, uint8_t code, u_int32_t values, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
