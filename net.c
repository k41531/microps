...
net_device_register(struct net_device *dev)
{
    ...
    dev->next = devices;
    devices = dev;
    infof("register net device: %s, type=0x%04x", dev->name, dev->type);
    return 0;
}
...
net_device_open(struct net_device *dev)
{
    if(NET_DEVICE_IS_UP(dev)){
        errorf("device %s is already opened", dev->name);
        return -1;
    }
    if(dev->ops->open){
...
{
    if(!NET_DEVICE_IS_UP(dev)){
        errorf("device %s is not opened", dev->name);
        return -1;
    }
    if(len > dev->mtu){
    ...
    if(dev->ops->transmit(dev, type, data, len, dst) == -1){
        errorf("device %s transmit failed, len=%zu", dev->name, len);
        return -1;
    }
    return 0;
}
