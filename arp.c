...
arp_cache_alloc(void)
{
    ...
    for (entry = caches; entry < tailof(caches); entry++){
        ...
        if (oldest == NULL || timercmp(&entry->timestamp, &oldest->timestamp, <)){
            oldest = entry;
        }
    }
...
arp_resolve(struct net_iface *iface, ip_addr_t pa, uint8_t *ha)
{
    ...
    if (iface->dev->type != NET_DEVICE_TYPE_ETHERNET){
        errorf("unsupported device type");
        return ARP_RESOLVE_ERROR;
    }
    if (iface->family != NET_IFACE_FAMILY_IP){
        errorf("unsupported interface family");
        return ARP_RESOLVE_ERROR;
    }
    mutex_lock(&mutex);
    cache = arp_cache_select(pa);
    ...
    if(cache->state == ARP_CACHE_STATE_INCOMPLETE){
        debugf("cache incomplete, pa=%s", ip_addr_ntop(pa, addr1, sizeof(addr1)));
        mutex_unlock(&mutex);
        arp_request(iface, pa);
        return ARP_RESOLVE_INCOMPLETE;
    }
