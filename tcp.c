...
tcp_pcb_release(struct tcp_pcb *pcb)
{
    ...
    debugf("release, local=%s, foreign=%s",
...
static void
tcp_segment_arrives(struct tcp_segment_info *seq, uint8_t flags, uint8_t *data, size_t len, struct ip_endpoint *local, struct ip_endpoint *foreign)
{
    struct tcp_pcb *pcb;
    ...
    if(!pcb || pcb->state == TCP_PCB_STATE_CLOSED) {
        ...
        if (!TCP_FLG_ISSET(flags, TCP_FLG_ACK)) {
            tcp_output_segment(0, seq->seq + seq->len, TCP_FLG_RST | TCP_FLG_ACK, 0, NULL, 0, local, foreign);
        } else {
            tcp_output_segment(seq->ack, 0, TCP_FLG_RST, 0, NULL, 0, local, foreign);
        }
        return;
    }
    switch (pcb->state) {
    ...
    case TCP_PCB_STATE_SYN_SENT:
        ...
        */
        ...
        /*
        * 3rd check the security and precedence 
        */
    ...
    */
    ...
    case TCP_PCB_STATE_SYN_RECEIVED:
        if (pcb->snd.una <= seq->ack && seq->ack <= pcb->snd.nxt) {
...
tcp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    if (src == IP_ADDR_BROADCAST || src == iface->broadcast || dst == IP_ADDR_BROADCAST || dst == iface->broadcast) {
        errorf("onl¥ supports unicast, src=%s, dst=%s",
    ...
    hlen = (hdr->off >> 4) << 2;
    seq.seq = ntoh32(hdr->seq);
    seq.ack = ntoh32(hdr->ack);
    seq.len = len - hlen;
    ...
    seq.wnd = ntoh16(hdr->wnd);
    seq.up = ntoh16(hdr->up);
    mutex_lock(&mutex);
    tcp_segment_arrives(&seq, hdr->flg, (uint8_t *)hdr + hlen, len - hlen, &local, &foreign);
    mutex_unlock(&mutex);
    return;
}
...
tcp_open_rfc793(struct ip_endpoint *local, struct ip_endpoint *foreign, int active) {
    ...
    if (active) {
        errorf("active open is not supported");
        tcp_pcb_release(pcb);
        mutex_unlock(&mutex);
        return -1;
    } else {
        debugf("passive open: local=%s, waiting forconnection...",
            ip_endpoint_ntop(local, ep1, sizeof(ep1)));
        pcb->local = *local;
        if(foreign){
            pcb->foreign = *foreign;
