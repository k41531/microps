...
int
ether_transmit_helper(struct net_device *dev, uint16_t type, const u_int8_t *data, size_t len, const void *dst, ether_transmit_func_t callback)
{
...
ether_input_helper(struct net_device *dev, ether_input_func_t callback)
{
    ...
    hdr = (struct ether_hdr *)frame;
    if(memcmp(dev->addr, hdr->dst, ETHER_ADDR_LEN) != 0 && memcmp(ETHER_ADDR_BROADCAST, hdr->dst, ETHER_ADDR_LEN) != 0){
        debugf("not for me");
        return -1;
    }
