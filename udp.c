...
#define UDP_SOUCRE_PORT_MIN 49152
#define UDP_SOUCRE_PORT_MAX 65535
...
udp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    if (cksum16((uint16_t *)hdr, len, psum) != 0) {
        errorf("checksum error: sum=0x%04x, verify=0x%04x", hdr->sum, cksum16((uint16_t *)hdr, len, -hdr->sum + psum));
        return;
    }
    ...
    if (!pcb) {
        mutex_unlock(&mutex);
        errorf("pcb not found: %s:%d", ip_addr_ntop(dst, addr1, sizeof(addr1)), ntoh16(hdr->dst));
        return;
    }
    entry = memory_alloc(sizeof(*entry) + len - sizeof(*hdr));
    if (!entry) {
        mutex_unlock(&mutex);
        errorf("momory_alloc() filure");
        return;
    }
...
udp_output(struct ip_endpoint *src, struct ip_endpoint *dst, const uint8_t *data, size_t len)
{
    ...
    if(len > IP_PAYLOAD_SIZE_MAX - sizeof(*hdr)) {
        errorf("too long payload: %zu", len);
        return -1;
    }
...
udp_sendto(int id, uint8_t *buf, size_t len, struct ip_endpoint *foreign)
{
    ...
    if (!pcb->local.port) {
        for (p = UDP_SOUCRE_PORT_MIN; p <= UDP_SOUCRE_PORT_MAX; p++) {
    ...
    mutex_unlock(&mutex);
    return udp_output(&local, foreign, buf, len); 
}
